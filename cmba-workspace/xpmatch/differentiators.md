...
| E. Ratings from real trips, tagged by taste ("travelers with your taste loved it") | 2 (an asset that only XPMatch's check-in loop produces) | Wanderlog has trips with dates and could add check-ins. Whoever builds taste density in a city first wins; no rival's model stops it | POSITIONAL, not built yet |
