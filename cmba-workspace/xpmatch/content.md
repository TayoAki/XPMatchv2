...
- **Why:** the traveler finds a plan at the moment of need, by searching "3 days in … itinerary", "how many days in …", "how to avoid tourist traps in …" (`validation.md`, search intent). Nobody follows a travel app's founder in order to plan a weekend.
- **What that means here:**
  - About 12 substantial assets over 90 days, ranked by how close the searcher is to planning *this* trip.
  - Each asset is a page on the landing site, plus spokes.
  - The founder's LinkedIn (`profile.md`) carries 1–2 spokes a week, not a daily calendar.
  - Forums and city communities get helpful answers, with no links where the rules ban promotion (RS does).
- **Unchanged from the default mode:** teach only, the matrix, trailer/meat/summary + CTC, curation, the hook bank and seasonal replay.

**Rules every asset follows (from AGENTS.md and the plan):**
- A real traveler's trip appears only with that traveler's permission **for web publication**. This is a separate scope from in-app matching (plan §7 `content_permission`). The contributor can withdraw it, and the page then drops it.
- No invented travelers, reviews, itineraries or statistics. An illustration is labeled as one.
- No traveler's dates, companions, spending or notes unless they chose to publish them.
- City assets wait for D-006 (the pilot city) and for at least 3 permissioned trips.

## Matrix

Themes (9) × styles; each cell is a headline.

| Theme \ Style | Actionable / step-by-step | Contrarian | X vs Y | Analytical | Listicle | Observation / prediction |
| --- | --- | --- | --- | --- | --- | --- |
...
### 18 headlines from the pairings

1. The 4-slot day: how to plan a city day that doesn't wear you out (T1, step-by-step)
2. "See everything" is how short trips go wrong (T1, contrarian)
3. 3 days or 4? What real trips to [pilot city] actually lasted (T1, X vs Y / analytical)
4. Write your 3 must-haves before you search for anything (T2, actionable)
...
18. How to share a trip so it actually helps the next traveler (T9, step-by-step; recruits contributors)

## 90-day plan: 12 assets ranked by intent depth

Rank 1 is the searcher closest to booking *this* trip. The build order differs from the rank, because city assets wait for D-006 and permissioned trips.
...
- **Asset 1** (3 days in [pilot city]) is the content priority, and `metrics.md` must treat it as the first content asset, not reject it.
- **Asset 7** is the first to ship, because it needs no city.

**Asset 11 moved earlier (coherence fix, 2026-09-23):**
- The first draft scheduled it for weeks 11–12. But assets 1–3 (weeks 5–8) each need at least 3 web-permissioned trips, and nothing before week 11 asked anyone for a trip.
- It now ships the day D-006 is decided, as the active ask that `metrics.md` tests in W1. The city assets start once W1 has produced 3 permissioned trips, so their build windows slip if D-006 does.
- Asset 1 must not launch during `metrics.md` W3.

**The method for asset 6** (so the comparison is fair and repeatable):
- Paste the same request, word for word, into the assistant with no follow-ups, on a stated date and model version.
- Check each stop against the place's own site for existence, opening days and area.
- Publish the prompt, the unedited output and every check.
- Never edit the AI's answer to make it look worse.
...
- **Stage:** problem-aware.
- **It landed if** comments name specific failures (closed places, cross-town zigzags).
- **It missed if** comments debate AI in general. Then Asset 7's page should lead with the checklist, not the argument.
...
**Meat:**
1. **Did they travel the way you do?** A 5★ from someone on a food pilgrimage means something different from a 5★ from someone who wanted a quick lunch.
2. **When did they go?** Menus, owners and prices change. Weigh last season over three years ago.
3. **What would they skip?** Good reviewers name the catch. A review with no catch is an advertisement or a mood.
4. **Would you have liked their other picks?** Scroll their other reviews. If you'd hate half of them, their 5★ tells you little.

**Summary:** Before trusting the stars, check four things about the reviewer: taste, timing, catches and track record.

**CTC:** Whose recommendations do you actually trust when you travel, and why them?
...
**CTC:** Been to [pilot city] recently? What would you tell the next person to skip?

**Testable expectation:**
- **Stage:** contributors.
- **It landed if** at least 3 people who've been to [pilot city] reply or DM. That's the supply signal that matters most (`idea.md`, binding constraint).
- **Before posting:** don't publish until D-006 fills the placeholder.
- **Measurement:** it's the active ask that `metrics.md` tests in W1. Post it on W1's first day, together with direct messages, and on no other date.

## Curation log

| Idea | Source | Query used |
| --- | --- | --- |
| 3-day plans, how many days | Google autocomplete | "how to plan a 3 day trip to", "how many days in" stems (`validation.md`) |
| AI itinerary checks | Google autocomplete | "can chatgpt plan a trip for me", "chatgpt travel itinerary prompt reddit" |
...
| Question sets | AnswerSocrates, AnswerThePublic | `GAP`: not reachable in this environment. Human task: run "[pilot city] itinerary" and "[pilot city] for couples" once D-006 is decided |
| Competitor content | Wanderlog guides, Mindtrip blog | `GAP`: not fetched for content. Human task: list the top 10 [pilot city] guides on each, and note what they leave out (catches, who wrote them) |
...
| # | Spoke | Style | Where |
| --- | --- | --- | --- |
| 1 | Day 1 as a story, from one contributor's trip (with permission) | Story | LinkedIn |
...
All are original and untested; none is copied from a named creator.
...
10. Your plan is a list of places. It isn't a trip yet.
11. Stop asking "where should I eat?"
12. The catch is the most useful part of a recommendation.
13. Four slots a day. Not eight.
14. If every stop is famous, the plan isn't yours.
15. A closed restaurant is the most expensive stop in any itinerary.
16. Two people, one trip, one list, if you do this first.
17. Before you trust a review, find the reviewer's other picks.
18. What real travelers skipped in [pilot city].
19. The morning slot is the only one you can count on.
20. We asked an AI to plan [pilot city]. Then we checked every stop.
21. Popular and right for you are different questions.
22. The review with no catch is the one to doubt.
23. How long real trips to [pilot city] actually lasted.
24. Share your trip so it helps the next person.

## Play the hits

**Rules:**
- Score every post 1–10 a week after publishing, on comments that answer the CTC, not likes.
- Repost anything scoring 7 or more at +182, +364, +546 and +728 days.
- City-in-month assets (asset 4) replay yearly, a month before that month, updated with the new season's trips.

| Post | Score | Repost 1 | Repost 2 | Repost 3 | Repost 4 |
| --- | --- | --- | --- | --- | --- |
| Post 1: check an AI itinerary | Not published yet | publish +182 d | +364 d | +546 d | +728 d |
| Post 2: the reviewer, not the stars | Not published yet | +182 d | +364 d | +546 d | +728 d |
| Post 3: the 4-slot day | Not published yet | +182 d | +364 d | +546 d | +728 d |
| Post 4: share a trip | Not published yet | +182 d | +364 d | +546 d | +728 d |
